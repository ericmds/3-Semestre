...
bool verificaOrdenacaoCrescente(Celula *lista) {
    for (Celula *p = lista; p->prox != NULL; p = p->prox) {  // Percorre até o penúltimo elemento
        if (p->dado > p->prox->dado) {
            return false;  // Se encontrar um elemento maior que o próximo, lista não está ordenada
        }
    }

    return true;  // Se não encontrar erros, a lista está ordenada de forma crescente
}
...
double calcularMedia(int dado, Celula *lista) {
